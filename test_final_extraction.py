...
def test_final_extraction():
    ...
    for filename in test_files:
        if os.path.exists(filename):
            ...
            # 修理手順の抽出テスト
            print("\n🔧 修理手順抽出テスト:")
            repair_steps = extract_repair_steps_final(content)
            if repair_steps:
                print("✅ 修理手順抽出成功:")
                print(repair_steps[:300] + "..." if len(repair_steps) > 300 else repair_steps)
            else:
                print("❌ 修理手順抽出失敗")
            
            # 注意事項の抽出テスト
            print("\n⚠️ 注意事項抽出テスト:")
            warnings = extract_warnings_final(content)
            if warnings:
                print("✅ 注意事項抽出成功:")
                print(warnings[:300] + "..." if len(warnings) > 300 else warnings)
            else:
                print("❌ 注意事項抽出失敗")
            
            # 費用目安の抽出テスト
            print("\n💰 費用目安抽出テスト:")
            costs = extract_cost_information_final(content)
            if costs:
                print("✅ 費用目安抽出成功:")
                print(costs[:300] + "..." if len(costs) > 300 else costs)
            else:
                print("❌ 費用目安抽出失敗")
        else:
            print(f"❌ {filename} が見つかりません")

def extract_repair_steps_final(content):
    """テキストから修理手順を抽出（最終版）"""
    import re
...
def extract_warnings_final(content):
    """テキストから注意事項を抽出（最終版）"""
    import re
...
def extract_cost_information_final(content):
    """テキストから修理費用を抽出（最終版）"""
    import re
